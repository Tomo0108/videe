...
useEffect(()=>{if(paused)return;const item=items.find(i=>(!i.thumbnail||i.thumbnailVersion!==THUMBNAIL_VERSION)&&!attempted.current.has(i.id));if(!item)return;let cancelled=false;let url='';const video=document.createElement('video');video.muted=true;video.preload='auto';let timer:ReturnType<typeof setTimeout>;let finish:()=>void=()=>{};
...
if(cancelled)return;const frame=captureFrame(video);attempted.current.add(item.id);update(item.id,{...frame,duration:Number.isFinite(video.duration)?video.duration:0});
}catch{if(!cancelled){attempted.current.add(item.id);update(item.id,{});}}finally{clearTimeout(timer);video.removeAttribute('src');video.load();if(url)URL.revokeObjectURL(url);}};void work();
