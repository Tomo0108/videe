...
const FlowButton = React.forwardRef<HTMLButtonElement, FlowButtonProps>(
  ({ children, size = 'default', borderColor = 'var(--rotating-border-color)', className, asChild = false, ...props }, ref) => {
    ...
    const createRoundedRectPath = (w: number, h: number, r: number) =>
      `M${r},0.5 H${w - r} A${r},${r} 0 0 1 ${w - 0.5},${r} V${h - r} A${r},${r} 0 0 1 ${w - r},${h - 0.5} H${r} A${r},${r} 0 0 1 0.5,${h - r} V${r} A${r},${r} 0 0 1 ${r},0.5 Z`;
    return (
      <div className="relative inline-block group pointer-events-none">
        ...
        >
          <svg width={dimensions.width} height={dimensions.height} viewBox={`0 0 ${dimensions.width} ${dimensions.height}`} aria-hidden="true" preserveAspectRatio="none" className="absolute top-0 left-0 w-full h-full pointer-events-none">
            <path
        ...
        <Comp
          ref={buttonRef}
          className={cn(
            "relative z-0 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 pointer-events-auto cursor-pointer inline-flex items-center justify-center gap-2 whitespace-nowrap font-[550] transition-[color,background-color,transform,box-shadow] duration-200 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 bg-neutral-100 dark:bg-muted/50 text-primary dark:hover:bg-transparent hover:bg-transparent",
            sizeMap[size],
            className,
          )}
          {...props}
