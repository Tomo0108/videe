...
const resize = (size,path) => execFileSync('sips',['-z',String(size),String(size),source,'--out',path],{stdio:'pipe'});
try {
  copyFileSync(source,resolve('public/icon.png'));
  for(const size of [32,180,192,512]) resize(size,join(output,`icon-${size}.png`));
  ...
  const images = sizes.map(size=>{const path=join(temp,`${size}.png`);resize(size,path);return readFileSync(path);});
