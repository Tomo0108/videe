...
import { defaults, normalizePreferences, nextOnEnded, stepSpeed } from '../src/preferences.mjs';
test('malformed settings recover to safe playback and UI values',()=>{
  ...
  assert.equal(settings.repeat,'off');assert.equal(settings.theme,'system');assert.equal(settings.layout,'list');
...
test('speed steps stay on the allowed scale',()=>{
  assert.equal(stepSpeed(1,1),1.25);
  assert.equal(stepSpeed(2,1),2);
  assert.equal(stepSpeed(0.5,-1),0.5);
  assert.equal(stepSpeed(99,1),1.25);
});
